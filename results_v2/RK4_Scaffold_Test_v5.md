...
def rk4(y, h):
    x = 0.0
    y_new = y + (h / 6) * (k1(x, y) + 2*k2(x, y) + 2*k3(x, y) + k4(x, y))
    return x, y_new
...
Traceback (most recent call last):
  File "C:\Users\SAVYAS~1\AppData\Local\Temp\tmpmbls1r8j.py", line 17, in <module>
    x, y = rk4(y, h)
           ^^^^^^^^^
  File "C:\Users\SAVYAS~1\AppData\Local\Temp\tmpmbls1r8j.py", line 5, in rk4
    y_new = y + (h / 6) * (k1(x, y) + 2*k2(x, y) + 2*k3(x, y) + k4(x, y))
                           ^^
...
## Surgical Audit
{
  ...
  "surgical_feedback": [
    {
      "line": "Missing definitions for k1, k2, k3, k4 in `rk4` function",
      "correction": "Add the following helper functions to compute the slopes (k1, k2, k3, k4) for the RK4 method. Replace the `rk4` function with this corrected version:\n\n```python\ndef k1(x, y):\n    return h * f(x, y)\n\ndef k2(x, y):\n    return h * f(x + h/2, y + k1(x, y)/2)\n\ndef k3(x, y):\n    return h * f(x + h/2, y + k2(x, y)/2)\n\ndef k4(x, y):\n    return h * f(x + h, y + k3(x, y))\n\ndef rk4(y, h):\n    x = 0.0\n    k1_val = k1(x, y)\n    k2_val = k2(x, y)\n    k3_val = k3(x, y)\n    k4_val = k4(x, y)\n    y_new = y + (h / 6) * (k1_val + 2*k2_val + 2*k3_val + k4_val)\n    x += h\n    return x, y_new\n```\n\nAdditionally, the `x` update in the `rk4` function was missing. The corrected version increments `x` by `h` in each step."
    },
    {
      "line": "Initial `x` update in the `while` loop",
      "correction": "The `x` variable should be updated in the `while` loop to reflect the step progression. Replace the `while` loop with:\n\n```python\nwhile x < 0.2:\n    x, y = rk4(y, h)\n```\n\nThis is already present in the code, but the `x` update in `rk4` is missing."
    }
  ],
  "survival_score": 3
}